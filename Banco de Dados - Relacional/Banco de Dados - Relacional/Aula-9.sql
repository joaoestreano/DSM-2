--Exercicio 1
CREATE OR REPLACE VIEW v_alunos_cursos AS
SELECT 
...
--Exercicio 2
CREATE OR REPLACE VIEW v_transacoes_recentes AS
SELECT 
    t.id_transacao,
    t.id_conta,
    t.data_transacao,
...
FROM 
    transacoes t
WHERE 
    t.data_transacao >= CURRENT_DATE - INTERVAL '30 days';
...
-- Criar uma view para mostrar os nomes dos projetos

CREATE VIEW v_nome_projetos(nome)
AS SELECT NOME FROM TBL_PROJETOS

SELECT *
FROM v_nome_projetos;
--
-- Criar uma view para listar quantidade de projetos por departamento ordenado do maior para o menor

CREATE VIEW v_qtde_projeto (nome, quantidade)
AS SELECT d.nome, COUNT(p.cod_projeto)
FROM tbl_projetos p
...
SELECT *
FROM v_qtde_projeto ORDER BY quantidade;

-- ALTERAÇÕES NA VIEW
ALTER VIEW v_nome_projetos RENAME TO v_nomes_projetos;

CREATE OR REPLACE VIEW v_nome_projetos AS
SELECT *
FROM tbl_projetos;
--
